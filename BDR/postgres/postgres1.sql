...
where emprestimo_id = 3;

-- procedures e triggers: atualizar o estoque automaticamente
-- procedure para atualizar estoque
create or replace function atualizar_estoque() 
returns trigger as $$
begin
	if (new.devolvido=True)
	update livros set estoque = estoque+1 where id_livro = id
	end if;

	if (new.devolvido=false)
	update livros set estoque = estoque-1 where id_livro = id
	end if;
return new;
end;
...
-- trigger que chama a procedure atualizar_estoque() ao devolver o livro
